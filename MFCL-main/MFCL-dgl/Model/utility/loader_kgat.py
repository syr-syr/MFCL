...
class KGAT_loader(Data):
    def __init__(self, args, path):
        super().__init__(args, path)        # super()调用父类
    ...
    def _bi_norm_lap(self, adj):           # 双向（bi）的归一化 Laplacian 矩阵的计算方法。通常是针对无向图的归一化方法，通过考虑节点的入度和出度来对称地归一化 Laplacian 矩阵。
    ...
    def _si_norm_lap(self, adj):           # 单向（si）的归一化 Laplacian 矩阵的计算方法。通常是针对有向图的归一化方法，它基于节点的出度进行归一化，旨在保持图的结构特性，并在图的表示中考虑节点的出度信息。
        rowsum = np.array(adj.sum(1))
    ...
    def _get_lap_list(self, is_subgraph = False, subgraph_adj = None):
        ...
        return lap_list         # 为Laplacian矩阵的列表
